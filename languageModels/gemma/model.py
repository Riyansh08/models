...
class Gemma(nn.Module):
    ...
    def forward(self, x, attention_mask=True, cache=None):
        batch_size, seq_len = x.shape
        x = self.embedding(x)
        x = x * math.sqrt(self.config.dim_size)
        ...
        if cache is None or cache.num_items() == 0:
            tri = torch.tril(torch.ones(seq_len, seq_len), dtype=torch.bool)
        else:
            assert x.size(1) == 1, "q_len must be 1 during caching"
            kv_len = cache.num_items() + 1
            tri = torch.ones((1, kv_len), dtype=torch.bool)
            attention_mask = tri.unsqueeze(1).unsqueeze(2)
        attention_mask = pad_mask + attention_mask
        x = x
        for layer_index, layer in enumerate(self.decoder_layers):
            x = layer(x, layer_index, attention_mask=attention_mask, cache=cache)
        x = self.layernorm(x)
        logits = self.output(x)
        return logits
#MODEL FOR LANGUAGE MODELLING
class LanguagModel(nn.Module):
    def __init__(self, config: GemmaConfig):
        super(LanguagModel, self).__init__()
        self.model = Gemma(config)
        self.register_buffer("_cache", KVCache())
    def forward(self, x, attention_mask=True, cache=None):
        return self.model(x, attention_mask=attention_mask, cache=cache)
